...
async def load_photo(message: types.Message,
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        await FSMADMIN.next()
        await message.reply("Send me title photo")
...
async def delete_data(message: types.Message):
    if message.from_user.id == ADMIN_ID:
        ...
        for result in inserting:
            await bot.send_photo(message.from_user.id, result[0], caption=f'Title: {result[1]}\n'
...
def register_handler_fsadmin(dp: Dispatcher):
    ...
    dp.register_message_handler(load_description, state=FSMADMIN.descriptoin)
